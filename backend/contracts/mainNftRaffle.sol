...
contract mainNftRaffle is IERC721Receiver {
    ...
    function createRaffle(string memory _raffleName, string memory _description, uint256 _nftPrice, uint256 _totalVolumeofTickets, uint256 _endTime, uint256 _nftId, address _nftContractAddress, string memory _nftSourceLink, address _charityAddress) public returns (uint256) {
    ...
    developmentTeam: payable(address(0xE46Cb9CA18520D3F0394F570C35259c188B33B98))
    });
    ...
    function getTotalTicketPrice (uint _raffleId, uint256 _totalTicketsWanted) public view returns (uint256) {
        Raffle storage raffle = raffles[_raffleId];
    ...
    function pickWinner(uint256 raffleId) public {
        ...
        require(raffle.totalSoldTickets >= raffle.totalVolumeofTickets * 80 / 100, "Raffle can't be completed yet");
        ...
        uint256 raffleCreatorPrize = raffles[raffleId].nftPrice + (rafflePrizePool.mul(5)).div(100);
        ...
        IERC721(raffle.nftContractAddress).safeTransferFrom(address(this), winningTicketOwner, raffle.nftId);
    ...
    function cancelRaffle(uint256 _raffleId) public {
        Raffle storage raffle = raffles[_raffleId];
        require(msg.sender == raffle.raffleCreator, "Only raffle creator can cancel the lottery");
        require(!raffle.raffleEnded, "Raffle has already ended");
        ...
        IERC721(raffle.nftContractAddress).safeTransferFrom(address(this), raffle.raffleCreator, raffle.nftId);
    ...
    function getPrizePool(uint256 _raffleId) public view returns (uint256, uint256, uint256, uint256) {
        uint256 rafflePrizePool = raffles[_raffleId].rafflePool;
        uint256 raffleCreatorPrize = raffles[_raffleId].nftPrice + (rafflePrizePool.mul(5)).div(100);
        uint256 developmentTeamPrize = (rafflePrizePool.mul(5)).div(100);
        uint256 charityPrize = rafflePrizePool - raffleCreatorPrize - developmentTeamPrize;
    ...
    function getRaffleWinner(uint256 raffleId) public view returns (address winner, uint256 winningTicket) {
        require(!raffles[raffleId].raffleCancelled, "Raffle has been cancelled");
        require(!raffles[raffleId].raffleEnded, "Raffle has already ended");
        require(raffles[raffleId].winningTicket != 0, "Winner has not been picked yet");

        return (raffles[raffleId].raffleWinner, raffles[raffleId].winningTicket);
    }
    ...
    function getEntityAddresses(uint256 _raffleId) public view returns (address, address, address) {

        return (raffles[_raffleId].raffleCreator, raffles[_raffleId].developmentTeam,raffles[_raffleId].charityAddress);
    }
