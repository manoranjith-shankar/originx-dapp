...
const BuyTickets = () => {
  ...
  const handleBuyTickets = async (event) => {
    ...
    try {
      ...
      // Convert raffleData.price to BigNumber object
      const ticketPriceWei = ethers.utils.parseUnits(raffleData.price, 18); // Assuming 18 decimal places for Ether
      console.log('ticketPriceWei:', ticketPriceWei.toString()); // Add this line
  
      // Calculate the total ticket price
      const totalTicketPriceWei = ticketPriceWei.mul(totalTicketsWanted);
      console.log('totalTicketPriceWei:', totalTicketPriceWei.toString()); // Add this line
  
      // Call the buyTicket function
      const txResponse = await contract.buyTicket(raffleData.id, totalTicketsWanted, {
